...
void main() {
    ...
    // vec4 baseColor = texture(textures[material.baseColorTextureID], inUV);
    // vec4 mr = texture(textures[material.metallicRoughnessTextureID], inUV);
    // vec4 normal =   texture(textures[material.normalTextureID], inUV);
    // vec4 emissive = texture(textures[material.emissiveTextureID], inUV);
