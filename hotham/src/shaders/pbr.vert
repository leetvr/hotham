...
layout (location = 0) out vec4 outStagePos;
...
void main() {
    ...
    if (d.skinID == NOT_PRESENT) {
        // Mesh has no skin
        outStagePos = d.stageFromLocal * vec4(inPos, 1.0);
        outNormal = normalize(inNormal * mat3(d.localFromStage));
    } else {
        // Mesh is skinned
        ...
        outStagePos = d.stageFromLocal * skinMatrix * vec4(inPos, 1.0);
        outNormal = normalize(mat3(skinMatrix) * inNormal * mat3(d.localFromStage));
    }

    outUV = inUV;
    outMaterialID = d.materialID;
    gl_Position = sceneData.viewProjection[gl_ViewIndex] * outStagePos;
}
